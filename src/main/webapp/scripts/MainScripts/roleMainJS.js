...
$(function(){
	$("#signOut").bind("click", function(){
		$.ajax({
			type: "GET",
			url: "/mavenHos/user/signOut"
		});
		window.location.replace("/mavenHos/");
		return false;
	});
});
...
function CreateInformationList(){
	...
	$.ajax({
		type: "POST",
		url: "/mavenHos/user/informationAjax",
		dataType: "json",
...
function createInformation(data, page, isHide){
	...
	for(var i = begin;i < end;++i){
		var $titleTd = $("<td><a href=news?id="+ data[i].id +" target=_blank>" + data[i].title + "</a></td>");
